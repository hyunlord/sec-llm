...
# HelpSteer2 was named as the replay top-up. Its licence is acceptable; its
# provenance is not, under the constraint that replay turns be human-written.
# Recorded as evaluated-and-not-used so the decision is inspectable. Nothing
# was downloaded, so there is no lock entry and no digest.
SOURCES["replay_helpsteer2_not_used"] = {
    "source_id": "replay_helpsteer2_not_used",
    "source_name": "nvidia/HelpSteer2 (evaluated as replay top-up -- NOT USED)",
    ...
    "used": False,
    "license": {
        "source_license": "Creative Commons Attribution 4.0 (declared in the dataset card: `license: cc-by-4.0`)",
        "source_license_url": "https://huggingface.co/datasets/nvidia/HelpSteer2/blob/main/README.md",
        "upstream_license": (
            "Prompts: mostly user-contributed ShareGPT plus ~5% written by Scale AI. Responses: generated by "
            "in-house NVIDIA LLMs. The response turns are therefore model-generated, not human-written."
        ),
        "redistribution_status": "permitted_with_attribution",
        "commercial_status": "permitted",
        "contains_third_party_content": True,
        "pii_policy": "not applicable -- not downloaded; would be subject to the P2 scan if it ever were",
        "license_evidence": (
            "Dataset card, read 2026-09-19 at commit 990b2711: `license: cc-by-4.0`. CC BY 4.0 section 2(a)(1) "
            "grants a licence to \"reproduce and Share the Licensed Material, in whole or in part; and produce, "
            "reproduce, and Share Adapted Material\", and the CC BY 4.0 legal code contains no NonCommercial "
            "restriction (unlike CC BY-NC); section 3(a) conditions Sharing on attribution. Commercial use is "
            "therefore permitted -- the legal code's own FAQ and the absence of the NC element establish it, and "
            "the word \"commercial\" appears in the licence text in the definition of NonCommercial, which this "
            "licence deliberately omits."
        ),
        "weights_release": "not_addressed",
        "weights_release_evidence": "CC BY 4.0 does not mention machine learning, training, or model weights.",
        "model_publication_status": "not_addressed",
        "checked_on": "2026-09-19",
        "why_not_used": (
            "The dataset card states under Source: \"Responses are generated by early versions of a mix of 10 "
            "different inhouse LLMs (note: none from properitary LLM providers such as OpenAI).\" Every response "
            "turn is model-generated. The replay constraint is English human-written turns only; using HelpSteer2 "
            "would leave prompts with no human-written response to pair them with. Rejected on provenance, not "
            "on licence."
        ),
    },
}
...
DOC_ORDER = SOURCE_ORDER + ["replay_oasst2", "replay_helpsteer2_not_used"]  # appears in the license matrix
